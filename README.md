...
# DSA Connect (ethers.js version) [![Build status](https://github.com/Instadapp/dsa-connect/actions/workflows/build.yml/badge.svg?branch=master&event=push)](https://github.com/Instadapp/dsa-connect/actions/workflows/build.yml)
...
To enable web3 calls via SDK, instantiate [ethers.js library](https://github.com/ethers-io/ethers.js/#installing)

```js
// in browser
if (window.ethereum) {
  provider = new new Web3Provider(window.ethereum);
} else {
  provider = new JsonRpcProvider(ETH_NODE_URL)
}
```
...
dsa.build({
  ...
  gasLimit: gasLimit,  // {gas: gas} modified in ethers.js version
  origin: origin,
  authority: authority,
})
```
...
|---------------|-----------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `gasPrice`    | *string/number/BigNumber* | The gas price in gwei. Mostly used in Node implementation to configure the transaction confirmation speed.                                                                     |
...
| `nonce`       | *string/number/BigNumber* | Nonce of your sender account. Mostly used in Node implementation to send transaction with a particular nonce either to override unconfirmed transaction or some other purpose. |
...
spells.add({
  ...
  args: [
    "0x6B175474E89094C44Da98b954EedeAC495271d0F",
    ethers.utils.parseUnits(100, 'ether'), // 100 * 10^18 wei
    0,
    0
  ]
})
...
```js
spells.cast({
  gasPrice: ethers.utils.parseUnits(gasPrice, 'ether'), // in gwei, used in node implementation.
...
|--------------------------|-----------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `gasPrice`               | *string/number/BigNumber* | The gas price in gwei. Mostly used in Node implementation to configure the transaction confirmation speed.                                                                     |
| `value`                  | *string/number/BigNumber* | Amount of ETH which you want to send along with the transaction (in wei).                                                                                                      |
| `nonce`                  | *string/number/BigNumber* | Nonce of your sender account. Mostly used in Node implementation to send transaction with a particular nonce either to override unconfirmed transaction or some other purpose. |
